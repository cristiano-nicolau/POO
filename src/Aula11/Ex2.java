...
public class Ex2 {
    public static void main(String[] args) {
		...
		try(PrintWriter fileInfoPublico = new PrintWriter(new File("src/Aula11/Infopublico.txt"))){
			...
			System.out.println("\n\nb)\n");
			System.out.println("Infopublico.txt criado com sucesso no seguinte diretório: src/Aula11/Infopublico.txt");
		}catch(IOException e) {
			System.err.printf("ERRO: %s\n", e.getMessage());
		}
		
		System.out.println("\n\nc)\n");
		ArrayList<ciaAtraso> atrasosMedios=new ArrayList<>(); 
		companhias.forEach((k,v) -> atrasosMedios.add(new ciaAtraso(v.get(0).toString()+";"+v.get(3).toString())));
		...
		try(PrintWriter fileCidades = new PrintWriter(new File("src/Aula11/cidades.txt"))){
			System.out.println("\n\nd)\n");
	        cidadesComparator bvc = new cidadesComparator(cidades);
			...
			fileCidades.close();
			System.out.println("\n\n\n");
			System.out.println("cidades.txt criado com sucesso no seguinte diretório: src/Aula11/cidades.txt");
		}catch(IOException e) {
			System.err.printf("ERRO: %s\n", e.getMessage());
		}
